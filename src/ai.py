...
地点范围：精确到区或商圈（例：上海·徐汇、北京·五道口、深圳·南山）。
技能水平：新手/入门/进阶/高手/教练。
...
个人亮点：英语专八、CPA 持证人、摄影 5 年、前职业运动员、会摄影、情绪稳定。
输出格式
[羽毛球,周末,上海·徐汇,进阶,e人,不限,23-26,AA,夜猫子,前职业运动员]
...
根据每个用户的tag，生成一个匹配度最高的搭子列表(用JSON输出)；
示例输入：
```txt
...
分析用户的简介和匹配到的搭子的简介，给出一些社交建议和这位搭子的配对好处和坏处，分点介绍，输出纯文本。（注意，这些搭子都是匹配好的，请勿输出适合或不适合）
示例输入：
用户：
```txt
